...
export default Ember.Component.extend({
  ...
  activeWaterChargeType: Ember.computed('waterChargeType','house.contract.meter_info.water_charge', function () {
    let type = this.get('house.contract.meter_info.water_charge');
  ...
  //当前已选的水费收费方式 eg. 按固定费用
  activeGasChargeType: Ember.computed('gasChargeType','house.contract.meter_info.gas_charge', function () {
    let type = this.get('house.contract.meter_info.gas_charge');
  ...
  activePowerChargeType: Ember.computed('powerChargeType','house.contract.meter_info.power_charge', function () {
  ...
  renew: Ember.computed('house.contract.owner_name', function () {
    return !!this.get('house.contract.owner_name');
  }),
  ...
  actions: {
    ...
    toggle(key){
      ...
      if (!expanding && !this.get('house.house')) {
        $.toastr.error("请先选择房源信息,房源地址!");
        return;
      }
    ...
    updateContract(contractId){
      ...
      data.house = this.get('house.house.id');
      ...
      let houseId = this.get('house.house.id');
      ajax(url + 'housecontracts/' + contractId, {
        method: 'patch',
        data: data
      }).then((res) => {
        $.toastr.success('修改成功');
        this.$(".xsd-modal-close").eq(0).trigger("click");
        this.sendAction("updateStatus", houseId, "house");
        //防止重复提交
